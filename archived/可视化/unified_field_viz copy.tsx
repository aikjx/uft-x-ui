...
const UnifiedFieldVisualization = () => {
  ...
  useEffect(() => {
    ...
    const createElectromagneticField = () => {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const arrow = new THREE.ArrowHelper(direction, origin, 2, 0x0088ff, 0.5, 0.3)
        group.add(arrow)
      }
  ...
  return (
    <div className="h-screen w-full bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
      <div className="mx-auto flex h-full max-w-7xl flex-col">
        ...
        <div className="mb-4 text-center">
          ...
          <p className="text-sm text-gray-300">Unified Field Theory Interactive Visualization</p>
        </div>
        ...
        <div className="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-4">
          ...
          <div className="lg:col-span-3">
            <Card className="h-full border-cyan-500/30 bg-black/40 backdrop-blur">
              <CardContent className="h-full p-0">
                <div ref={mountRef} className="h-full w-full rounded-lg" />

                {/* Controls Overlay */}
                <div className="absolute bottom-8 left-1/2 flex -translate-x-1/2 transform gap-3">
                  <button
                    onClick={() => setIsRotating(!isRotating)}
                    className="rounded-lg bg-cyan-500/80 px-4 py-2 text-white backdrop-blur transition hover:bg-cyan-400"
                  >
                    {isRotating ? '⏸ 暂停' : '▶ 旋转'}
                  </button>
                  <button
                    onClick={() => setShowVectors(!showVectors)}
                    className="rounded-lg bg-purple-500/80 px-4 py-2 text-white backdrop-blur transition hover:bg-purple-400"
                  >
          ...
          {/* Formula Panel */}
          <div className="max-h-[calc(100vh-120px)] space-y-3 overflow-y-auto lg:col-span-1">
            {formulas.map(formula => (
              <Card
                key={formula.id}
                className={`transform cursor-pointer transition-all hover:scale-105 ${
                  selectedFormula === formula.id
                    ? 'border-cyan-400 bg-gradient-to-br from-cyan-500/30 to-purple-500/30 shadow-lg shadow-cyan-500/50'
                    : 'border-gray-700 bg-black/40 hover:border-cyan-500/50'
                } backdrop-blur`}
                onClick={() => setSelectedFormula(formula.id)}
              >
                <CardContent className="p-4">
                  <h3 className="mb-1 text-lg font-bold text-cyan-300">{formula.name}</h3>
                  <p className="text-sm text-gray-400">{formula.desc}</p>
                  {selectedFormula === formula.id && (
                    <div className="mt-2 border-t border-cyan-500/30 pt-2">
            ...
            {/* Info Card */}
            <Card className="border-amber-500/30 bg-gradient-to-br from-amber-500/20 to-orange-500/20 backdrop-blur">
              <CardContent className="p-4">
                <h3 className="mb-2 text-sm font-bold text-amber-300">💡 交互提示</h3>
