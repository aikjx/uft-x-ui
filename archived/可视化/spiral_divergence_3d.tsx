      ...
      if (isAnimating) {
        ...
        spiralSystems.forEach((system, sysIndex) => {
          ...
          system.particles.forEach((particle, particleIndex) => {
            ...
            // 计算粒子在圆柱截面上的位置
            const radialOffset = perpendicular1
              .clone()
    ...
    <div className="flex h-screen w-full flex-col bg-gray-900">
      <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 p-6 text-white">
        ...
        <div className="grid grid-cols-1 gap-2 text-sm opacity-90 md:grid-cols-3">
          <p>
            • 时空同一化: <span className="font-mono">r⃗(t) = C⃗t</span>
          </p>
          <p>
            • 三维螺旋: <span className="font-mono">r⃗(t) = r·cos(ωt)i⃗ + r·sin(ωt)j⃗ + ht·k⃗</span>
          </p>
          <p>
            • 质量定义: <span className="font-mono">m = k·(dn/dΩ)</span>
          </p>
        </div>
      </div>

      <div className="relative flex-1" ref={mountRef}>
        <div className="absolute right-4 top-4 z-10 max-w-xs space-y-3 rounded-lg bg-black bg-opacity-80 p-4 text-white">
          <h3 className="border-b border-gray-600 pb-2 text-lg font-bold">控制面板</h3>

          <div>
            <label className="mb-1 block text-sm">运动速度: {speed.toFixed(1)}x</label>
            <input
              type="range"
              min="0.1"
              max="3"
          ...
          <div className="flex items-center space-x-2">
            <input
              ...
              className="h-4 w-4"
            />
            <label htmlFor="cylinders" className="text-sm">
              显示圆柱体框架
            </label>
          </div>
          ...
          <button
            ...
            className="w-full rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-2 font-semibold transition-all hover:from-blue-700 hover:to-purple-700"
          >
            {isAnimating ? '⏸ 暂停' : '▶ 播放'}
          </button>

          <div className="mt-4 space-y-2 border-t border-gray-600 pt-3 text-xs">
        ...
        <div className="absolute bottom-4 left-4 z-10 rounded-lg bg-black bg-opacity-80 p-3 text-xs text-white">
