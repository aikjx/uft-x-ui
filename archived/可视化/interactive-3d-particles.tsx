...
const ParticleSystem = () => {
  ...
  useEffect(() => {
    ...
    scene.add(particles);
    particlesRef.current = { particles, geometry, targetPositions, basePositions: positionArray.slice() };
              ...
              if (video.readyState === video.HAVE_ENOUGH_DATA) {
                ...
                if (skinPixels.length > 50) {
                  ...
                  if (leftHandPixels.length > 20 && rightHandPixels.length > 20) {
                    ...
                    const normalizedDistance = Math.max(0.3, Math.min(2.0, (distance - 200) / 200 + 0.5));
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      {/* 调试视图：显示摄像头画面 */}
      {isHandDetectionActive && (
        <div className="absolute top-4 right-4 bg-black/80 p-2 rounded-lg z-20">
          <canvas 
            ref={canvasRef} 
            width="320" 
            height="240" 
          ...
          />
          <p className="text-white text-xs text-center mt-1">手势检测预览</p>
        </div>
      )}
      ...
      <button
        ...
        className="absolute top-4 left-4 bg-white/10 backdrop-blur-md text-white px-4 py-2 rounded-lg hover:bg-white/20 transition-all z-20"
      >
      ...
      {/* 控制面板 */}
      {showControls && (
        <div className="absolute top-4 left-20 bg-black/70 backdrop-blur-md text-white p-6 rounded-2xl shadow-2xl max-w-sm z-10">
          <h2 className="text-xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-cyan-400">
            粒子系统控制
          </h2>
            ...
            <div className="mt-2 space-y-1">
              <div className="flex items-center gap-2 text-xs">
                <div className={`w-2 h-2 rounded-full ${
                  cameraStatus === '检测到双手 ✓' ? 'bg-green-400' :
                  cameraStatus === '检测中...' || cameraStatus === '摄像头已启动' ? 'bg-yellow-400' :
                  cameraStatus === '启动失败' || cameraStatus === '错误' ? 'bg-red-400' :
                  'bg-gray-400'
                }`} />
          ...
          {/* 形状选择 */}
          <div className="mb-4">
            <label className="block text-sm mb-2">形状选择</label>
          ...
          {/* 图片上传 */}
          <div className="mb-4">
            <label className="block text-sm mb-2">上传图片生成形状</label>
            <input
              type="file"
              accept="image/*"
              onChange={handleImageUpload}
              className="text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gradient-to-r file:from-pink-500 file:to-cyan-500 file:text-white file:cursor-pointer hover:file:opacity-80"
            />
          </div>
            ...
            <div className="flex gap-2 items-center">
              ...
              {useGradient && (
                <>
                  ...
                  <div>
                    <label className="block text-xs mb-1">结束</label>
                    <input
                      type="color"
                      value={color2}
      ...
      <button
        ...
        className="absolute bottom-4 right-4 bg-white/10 backdrop-blur-md text-white px-6 py-3 rounded-full hover:bg-white/20 transition-all shadow-lg z-20"
      >
      ...
      {isHandDetectionActive && !cameraError && (
        <div className="absolute bottom-20 left-4 bg-cyan-500/20 backdrop-blur-md text-cyan-400 px-4 py-2 rounded-lg text-sm z-20">
      ...
      {/* 摄像头故障排除提示 */}
      {cameraError && (
        <div className="absolute bottom-20 left-4 bg-red-500/20 backdrop-blur-md text-white px-4 py-3 rounded-lg text-sm z-20 max-w-md">
          <div className="font-bold mb-2">📷 摄像头故障排除：</div>
