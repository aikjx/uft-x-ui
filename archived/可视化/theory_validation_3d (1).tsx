...
export default function TheoryValidation3D() {
  ...
  const drawParticleModel = (ctx, centerX, centerY, time) => {
    ...
    drawFormulaValidation(
      ctx,
      leftCenter - 100,
      centerY + 100,
      electronOmega * electronR,
      electronLinearV,
      3.0,
      true
    )
  ...
  const drawGalaxyModel = (ctx, centerX, centerY, time) => {
    ...
    const calculatedTotal = Math.sqrt(
      galaxyRotationSpeed * galaxyRotationSpeed + expansionSpeed * expansionSpeed
    )
  ...
  // 绘制粒子可视化
  const drawParticleVisualization = (
    ctx,
    centerX,
    centerY,
    radius,
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-gray-900">
      ...
      <canvas
        ref={canvasRef}
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform rounded border border-gray-600"
      />

      {/* 标题和状态 */}
      <div
        className={`absolute left-1/2 top-4 -translate-x-1/2 transform rounded-lg border-2 p-4 ${
          currentConfig.status === 'correct'
            ? 'border-green-500 bg-green-900'
            : currentConfig.status === 'mostly-correct'
              ? 'border-yellow-500 bg-yellow-900'
              : 'border-red-500 bg-red-900'
        }`}
      >
        <h2 className="text-center text-xl font-bold text-white">{currentConfig.title}</h2>
        <p className="mt-1 text-center text-sm text-gray-300">{currentConfig.subtitle}</p>
        <p className="mt-1 text-center text-xs text-gray-400">{currentConfig.description}</p>
      </div>

      {/* 控制面板 */}
      <div className="absolute left-4 top-4 max-w-xs rounded-lg border border-gray-600 bg-black bg-opacity-80 p-4">
        <h3 className="mb-3 text-lg font-bold text-white">论述验证切换</h3>
          ...
          {viewConfigs.map((config, index) => (
            ...
            >
              <div className="text-left">
                ...
                <div className="mt-1 text-xs opacity-75">{config.description}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* 详细说明面板 */}
      <div className="absolute bottom-4 right-4 max-w-md rounded-lg border border-gray-600 bg-black bg-opacity-80 p-4">
        <h4 className="text-md mb-2 font-bold text-yellow-400">详细分析</h4>
      ...
      {/* 图例 */}
      <div className="absolute right-4 top-4 rounded-lg border border-gray-600 bg-black bg-opacity-80 p-3">
        <h4 className="mb-2 text-sm font-bold text-white">图例</h4>
        <div className="space-y-1 text-xs text-white">
          <div className="flex items-center">
            <div className="mr-2 h-1 w-3 bg-red-500"></div>
            <span>旋转速度</span>
          </div>
          <div className="flex items-center">
            <div className="mr-2 h-1 w-3 bg-green-500"></div>
            <span>线性/发散速度</span>
          </div>
          <div className="flex items-center">
            <div className="mr-2 h-1 w-3 bg-blue-500"></div>
            <span>总速度</span>
          </div>
          <div className="flex items-center">
            <div className="mr-2 h-1 w-3 bg-blue-400"></div>
            <span>运动轨迹</span>
          </div>
        </div>
      </div>
