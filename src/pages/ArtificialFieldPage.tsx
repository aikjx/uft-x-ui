...
const ArtificialFieldPage: React.FC = () => {
  ...
  const createTimePotentialVisualization = (scene: THREE.Scene) => {
    ...
    // 水平网格
    const horizontalGrid = new THREE.GridHelper(size, divisions, 0x1a1a3a, 0x333366)
    gridGroup.add(horizontalGrid)

    // 垂直网格
    const verticalGrid = new THREE.GridHelper(size, divisions, 0x1a1a3a, 0x333366)
      ...
      >
        <div className="container mx-auto h-full px-0">
          ...
          <div className="absolute left-0 right-0 top-0 z-50 bg-gradient-to-b from-[#0a0a14]/80 to-transparent p-2 backdrop-blur-md">
            ...
            {/* 标签页切换 - 改进样式和交互 */}
            <motion.div
              className="mx-auto mt-2 flex max-w-3xl justify-center overflow-x-auto border-b border-blue-900/30"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {tabs.map(tab => (
                <motion.button
                  ...
                  className={`whitespace-nowrap px-4 py-2 text-sm font-medium transition-all duration-300 md:text-base ${activeTab === tab.id ? 'border-b-2 border-blue-500 text-blue-300 shadow-lg shadow-blue-900/10' : 'text-blue-100/70 hover:text-blue-200'}`}
          ...
          >
            {isLoading && (
              <motion.div
                className="absolute inset-0 z-10 flex items-center justify-center bg-[#0a0a14]/80"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              >
          ...
          {/* 详细说明面板 - 可折叠设计，默认隐藏 */}
          <motion.div
            className="max-h-1/3 absolute bottom-0 left-0 right-0 z-40 overflow-y-auto bg-gradient-to-t from-[#0a0a14]/95 to-transparent p-2 backdrop-blur-md"
            initial={{ opacity: 0, y: 100 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.3 }}
          >
            <motion.h2
              className="mb-2 border-l-4 border-blue-500 pl-2 text-xl font-bold text-blue-300 md:text-2xl"
              variants={itemVariants}
            >
            ...
            {activeTab === 'principles' && (
              <motion.div variants={containerVariants}>
                <motion.p
                  className="mb-2 text-sm leading-relaxed text-blue-100/80"
                  variants={itemVariants}
                >
                ...
                <motion.div
                  className={cn(
                    'relative overflow-hidden rounded-lg border border-blue-800/30 bg-[#0a0a14] p-2 shadow-inner'
                  )}
              ...
              <motion.div variants={containerVariants}>
                ...
                >
                  ...
                  <div className="relative z-10 text-sm text-blue-300">
                    时间膨胀效应:
                    <MathJax
                      formula="\Delta t' = \frac{\Delta t}{\sqrt{1 - \frac{v^2}{c^2}}}"
                      className="inline"
                    />
                  </div>
                </motion.div>
