...
const FormulaVisualizationPage: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      <div className="mx-0 flex h-full w-full flex-col gap-0 px-0 lg:flex-row lg:gap-0">
        {/* 左侧公式列表 - 优化响应式布局，最小化占据空间 */}
        <motion.div
          className={`${isMobileMenuOpen ? 'fixed inset-0 z-50' : 'lg:w-72'} sticky top-0 z-10 flex h-fit flex-col overflow-hidden rounded-r-2xl border border-blue-900/30 bg-opacity-95 bg-gradient-to-br from-[#121228] to-[#0f0f20] p-2 shadow-xl shadow-blue-900/5 backdrop-blur-sm transition-all duration-300 lg:max-h-[100vh]`}
        ...
        >
          ...
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm text-blue-200/60">19个核心公式的3D可视化</p>
          </div>
          <div className="scrollbar-thin scrollbar-thumb-blue-900/30 scrollbar-track-transparent flex-1 space-y-2 overflow-y-auto pr-2">
            {formulas.map(formula => (
              ...
              >
                <div className="mb-1 flex items-center gap-2 font-medium">
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-600/30 text-xs text-blue-400">
                    {formula.id}
                  </span>
                  {formula.name}
                </div>
        ...
        <motion.div className="flex flex-1 flex-col p-1" variants={itemVariants}>
          {/* 移动端菜单按钮 */}
          <div className="mb-2 flex justify-start lg:hidden">
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="rounded-md bg-gradient-to-r from-blue-600 to-cyan-600 px-3 py-1.5 text-xs font-medium text-white shadow-md shadow-blue-900/20 transition-all duration-300 hover:from-blue-700 hover:to-cyan-700"
            >
              📋 公式列表
            </button>
          </div>
          ...
          {/* 公式详情 - 极度精简，突出核心信息 */}
          <motion.div
            className="mb-2 rounded-xl border border-blue-900/30 bg-gradient-to-br from-[#121228] to-[#0f0f20] p-3 shadow-lg shadow-blue-900/10 transition-all duration-300"
          ...
          >
            <div className="mb-3 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
              <h3 className="flex items-center gap-2 bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-xl font-bold text-transparent sm:text-2xl">
              ...
              {showDerivative ? (
                <>
                  ...
                  {formulaService.deriveFormula(selectedFormula.id) ? (
                    ...
                    <div className="rounded-md bg-blue-900/10 py-4 text-center text-sm italic text-blue-200/60">
          ...
          {/* 可视化控制面板 */}
          <motion.div
            className="mb-2 rounded-xl border border-blue-900/30 bg-gradient-to-br from-[#121228] to-[#0f0f20] p-2 shadow-lg shadow-blue-900/10 transition-all duration-300"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex flex-wrap gap-2">
              {/* 显示网格 */}
              <label className="flex cursor-pointer items-center gap-1 text-xs text-blue-100/80">
                <input
                  type="checkbox"
              ...
              {/* 动画速度 */}
              <div className="ml-2 flex items-center gap-1 text-xs text-blue-100/80">
                <span>速度:</span>
                <input
                  type="range"
                  min="0.1"
                  ...
                  className="h-1 w-20 cursor-pointer appearance-none rounded-lg bg-blue-900/50 accent-blue-500"
                />
