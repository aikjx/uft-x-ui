...
const InteractiveExplorationPage: React.FC = () => {
  ...
  const handleParameterChange = useCallback(
    (simulation: string, paramName: string, value: number) => {
  ...
  const createSpacetimeSimulation = (scene: THREE.Scene) => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      points.push(new THREE.Vector3(radius * Math.cos(angle), radius * Math.sin(angle), i * 0.05))
    }
  ...
  const createGravitySimulation = (scene: THREE.Scene) => {
    ...
    scene.userData.update = () => {
      ...
      orbiter.position.set(r * Math.cos(angle), 0, r * Math.sin(angle))

      // 更新引力场粒子
      fieldParticles.children.forEach((particle: any) => {
        particle.userData.angle +=
          0.001 * (parameters.gravity.mass / Math.pow(particle.userData.radius, 2))
        particle.position.set(
        ...
        ;(particle.material as THREE.MeshBasicMaterial).color.setRGB(0.27, 0.72, 0.82 * intensity)
      })
    ...
    return paramConfig[activeSimulation as keyof typeof paramConfig].map(param => (
      <div key={param.name} className="space-y-1">
        ...
        <input
          ...
          onChange={e =>
            handleParameterChange(activeSimulation, param.name, parseFloat(e.target.value))
          }
          className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-blue-900/30 accent-blue-500 transition-all duration-300 hover:accent-blue-400"
          aria-label={`调整${param.label}`}
        />
        <div className="text-right text-xs text-blue-400">
          {parameters[activeSimulation][param.name].toFixed(1)}
          {param.unit}
        </div>
      </div>
    ))
  ...
  const renderSimulationDescription = () => {
    ...
    return (
      <div>
        <h2 className="mb-2 flex items-center gap-2 text-xl font-bold text-blue-300">
          <span className="inline-block h-6 w-2 rounded-full bg-blue-500"></span>
          {desc.title}
        </h2>
        {desc.content.map((paragraph, index) => (
          <p key={index} className="mb-2 text-sm leading-relaxed text-blue-100/80">
            {paragraph}
          </p>
        ))}
        <div className="rounded-lg border border-blue-800/30 bg-[#0a0a14] p-2 font-mono text-xs text-blue-300 shadow-inner shadow-blue-900/10">
          {desc.formula}
        </div>
      </div>
    )
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full w-full">
        ...
        {/* 加载指示器 */}
        {isLoading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#0a0a14]/80">
            <motion.div
        ...
        {/* 控制面板切换按钮 */}
        <div className="absolute left-2 top-2 z-40 flex gap-2">
          <motion.button
        ...
        <AnimatePresence>
          {showControls && (
            <motion.div
              className="absolute left-2 top-16 z-40 max-h-[80vh] max-w-[300px] overflow-y-auto rounded-lg bg-gradient-to-r from-[#0a0a14]/95 to-transparent p-4 backdrop-blur-md"
              initial={{ opacity: 0, x: -100 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -100 }}
              transition={{ duration: 0.3 }}
            >
              <h2 className="mb-4 flex items-center gap-2 text-lg font-bold text-blue-200">
              ...
              {/* 模拟类型选择 */}
              <div className="mb-4">
                <h3 className="mb-2 text-base font-medium text-blue-300">选择模拟类型</h3>
                <div className="space-y-1">
                  {SIMULATION_TYPES.map(sim => (
                    <motion.button
                      key={sim.id}
                      onClick={() => handleSimulationChange(sim.id)}
                      className={`w-full rounded-md p-2 text-left transition-all duration-300 ${activeSimulation === sim.id ? 'border-l-3 border-blue-500 bg-blue-600/20 text-blue-300' : 'text-blue-100/70 hover:bg-blue-900/20'}`}
              ...
              {/* 参数控制 */}
              <div className="mb-4">
                <h3 className="mb-2 text-base font-medium text-blue-300">参数调整</h3>
                <div className="space-y-3">{renderParameterControls()}</div>
              </div>
              ...
              <div className="mt-4 flex gap-2">
                <motion.button
                  onClick={saveScene}
                  className="flex flex-1 items-center justify-center gap-1 rounded-md bg-blue-600 px-3 py-2 text-sm text-white transition-colors duration-300 hover:bg-blue-700"
                ...
                <motion.button
                  ...
                  className="flex flex-1 items-center justify-center gap-1 rounded-md border border-blue-600 bg-transparent px-3 py-2 text-sm text-blue-300 transition-colors duration-300 hover:bg-blue-900/20"
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                  aria-label="重置所有参数"
                >
        ...
        <AnimatePresence>
          {showDescription && (
            <motion.div
              className="absolute bottom-2 left-0 right-0 z-40 max-h-[30vh] overflow-y-auto bg-gradient-to-t from-[#0a0a14]/95 to-transparent p-3 backdrop-blur-md"
