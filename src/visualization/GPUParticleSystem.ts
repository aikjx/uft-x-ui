...
export class GPUParticleSystem {
  ...
  constructor(scene: THREE.Scene, config: GPUParticleSystemConfig) {
    ...
    this.geometry.setAttribute(
      'lifeProgress',
      new THREE.BufferAttribute(this.lifeProgress, 1).setUsage(THREE.DynamicDrawUsage)
    )
    this.geometry.setAttribute(
      'active',
      new THREE.BufferAttribute(this.active, 1).setUsage(THREE.DynamicDrawUsage)
    )
  ...
  private emitParticleAt(index: number) {
    ...
    const speed =
      this.config.velocity.length() * (1 + (random3 - 0.5) * this.config.velocityVariance)
          ...
          this.config.rate *
            (1 -
              (this.particleCount / this.maxParticles) *
                this.performanceConfig.emissionAdjustmentFactor)
        )
      )
  ...
  public checkAutoPause() {
    ...
    // 如果在暂停状态但有更新请求，恢复
      ...
      this.isPaused &&
      now - this.lastUpdateTime <= this.performanceConfig.autoPauseThreshold
    ) {
      this.resume()
    }
  }
  ...
  private swapParticles(index1: number, index2: number): void {
    ...
    ;[this.positions[base1 + 1], this.positions[base2 + 1]] = [
      this.positions[base2 + 1],
      this.positions[base1 + 1]
    ]
    ;[this.positions[base1 + 2], this.positions[base2 + 2]] = [
      this.positions[base2 + 2],
      this.positions[base1 + 2]
    ]

    // 交换速度
    ;[this.velocities[base1], this.velocities[base2]] = [
      this.velocities[base2],
      this.velocities[base1]
    ]
    ;[this.velocities[base1 + 1], this.velocities[base2 + 1]] = [
      this.velocities[base2 + 1],
      this.velocities[base1 + 1]
    ]
    ;[this.velocities[base1 + 2], this.velocities[base2 + 2]] = [
      this.velocities[base2 + 2],
      this.velocities[base1 + 2]
    ]

    // 交换颜色
    ;[this.colors[base1], this.colors[base2]] = [this.colors[base2], this.colors[base1]]
    ;[this.colors[base1 + 1], this.colors[base2 + 1]] = [
      this.colors[base2 + 1],
      this.colors[base1 + 1]
    ]
    ;[this.colors[base1 + 2], this.colors[base2 + 2]] = [
      this.colors[base2 + 2],
      this.colors[base1 + 2]
    ]
    ...
    // 交换生命周期和年龄
    ;[this.lifetimes[index1], this.lifetimes[index2]] = [
      this.lifetimes[index2],
      this.lifetimes[index1]
    ]
    ;[this.ages[index1], this.ages[index2]] = [this.ages[index2], this.ages[index1]]
    ;[this.lifeProgress[index1], this.lifeProgress[index2]] = [
      this.lifeProgress[index2],
      this.lifeProgress[index1]
    ]
...
export class GPUParticleSystemManager {
   ...
   * 批量创建粒子系统
   */
  public createParticleSystems(
    systemsConfig: Record<string, GPUParticleSystemConfig>
  ): Map<string, GPUParticleSystem> {
   ...
   * 批量更新粒子系统配置
   */
  public updateParticleSystemConfigs(
    configs: Record<string, Partial<GPUParticleSystemConfig>>
  ): void {
