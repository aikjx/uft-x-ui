...
export class ServiceContainer {
  ...
  public register<T>(
    ...
    const { lifetime = ServiceLifetime.SINGLETON, dependencies = [], factory } = options || {}
 ...
 * 可注入装饰器 - 标记类为可注入服务
 */
export function Injectable(options?: { lifetime?: ServiceLifetime; dependencies?: any[] }) {
...
export function InjectProperty(token: string | symbol | Function) {
  return function (target: any, propertyKey: string | symbol) {
    ...
    const propertyDependencies =
      Reflect.getMetadata('propertyDependencies', target.constructor) || {}
