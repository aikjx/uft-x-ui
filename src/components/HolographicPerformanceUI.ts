...
export class HolographicPerformanceUI {
  ...
  private createHolographicPanels(): void {
    const panelTypes: Array<HologramData['type']> = [
      'cpu',
      'memory',
      'fps',
      'gpu',
  ...
  private addWarningEffect(panel: THREE.Mesh, severity: 'warning' | 'critical'): void {
    ...
    const animateWarning = () => {
      ...
      warningRing.material.opacity =
        (isCritical ? 0.8 : 0.4) * (0.5 + 0.5 * Math.sin(performance.now() * 0.005))
  ...
  private updateScanLines(): void {
    ...
    if (this.scanBeams.length === 0) {
      for (let i = 0; i < 6; i++) {
        const points = [new THREE.Vector3(-5, scanY, -5), new THREE.Vector3(5, scanY, -5)]
