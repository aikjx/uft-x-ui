...
const FormulaDisplay: React.FC<{
  ...
  const containerVariants = {
    ...
    hover: {
      scale: 1.03,
      boxShadow: displayConfig.cardShadow ? `0 20px 60px -10px ${colors.glow}40` : '0 10px 30px -5px rgba(0, 0, 0, 0.3)',
      transition: {
        duration: 0.3,
        ease: [0.22, 1, 0.36, 1]
      }
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3 text-sm">
          {formula.variables.map((variable, index) => (
            <motion.div 
              key={index} 
              className="flex items-start gap-3 p-3 rounded-lg bg-opacity-10 backdrop-blur-sm" 
            ...
            >
              <span className="font-mono font-bold text-lg" style={{ color: colors.variable }}>{variable.name}</span>
              <div className="flex-1">
                <span className="block" style={{ color: colors.text }}>{variable.description}</span>
                <span className="text-xs font-mono opacity-75 mt-1 block" style={{ color: colors.text }}>({variable.unit})</span>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      {/* 背景装饰 */}
      <div className="absolute -top-20 -right-20 w-40 h-40 rounded-full bg-opacity-10 blur-3xl" style={{ backgroundColor: colors.glow }} />
      <div className="absolute -bottom-20 -left-20 w-40 h-40 rounded-full bg-opacity-10 blur-3xl" style={{ backgroundColor: colors.variable }} />
      
      {/* 公式卡片头部 */}
      <motion.div 
        className="relative flex items-center justify-between px-6 py-4 border-b bg-opacity-50 backdrop-blur-sm"
        ...
        <div className="flex items-center gap-2">
          ...
          {displayConfig.showCopyButton && (
            <motion.button
              ...
              className="p-2 rounded-full hover:bg-opacity-20 transition-all duration-300 relative overflow-hidden"
              style={{ backgroundColor: `${colors.border}10` }}
              title="复制公式"
              variants={iconVariants}
              whileHover="hover"
