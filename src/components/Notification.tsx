  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ref={notificationRef}
          className={`relative rounded-lg border p-4 ${style.bgColor} ${style.borderColor} cursor-pointer shadow-lg shadow-blue-900/20 backdrop-blur-sm`}
          custom={index}
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className={`text-sm ${style.textColor} break-words leading-relaxed`}>{message}</p>
            </div>
            <button
              className="ml-2 flex-shrink-0 rounded-full p-1 text-blue-200/50 transition-colors hover:text-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
              onClick={e => {
                e.stopPropagation()
                handleClose()
              }}
...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({
  ...
  return (
    <NotificationContext.Provider value={{ showNotification, clearAllNotifications }}>
      {children}
      <div
        className={`pointer-events-none fixed z-50 w-full max-w-sm ${NOTIFICATION_POSITIONS[position]}`}
        role="region"
        aria-live="polite"
      >
