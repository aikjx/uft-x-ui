...
export class AutomatedPerformanceOptimizer {
  ...
  private findOptimalParametersFromHistory(): Record<string, number> | null {
    ...
    goodHistory.forEach(entry => {
      ...
      // 统计关键参数
      const keyParams = [
        'particleCount', 'renderScale', 'shadowQuality', 'postProcessing',
        'textureMemory', 'drawCalls', 'triangleCount'
      ];
    ...
    for (const [param, stats] of Object.entries(paramStats)) {
      if (stats.count > 0) {
        ...
        switch (param) {
          ...
          case 'shadowQuality':
            // 阴影质量：根据设备性能调整
            optimalParams[param] = this.devicePerformanceLevel >= DevicePerformanceLevel.HIGH ? 1.0 : 0.5;
            break;
          case 'textureMemory':
            // 纹理内存：使用平均值
            optimalParams[param] = Math.round((stats.sum / stats.count) * 100) / 100;
   ...
   * 优化帧率瓶颈
   */
  private optimizeFPSBottleneck(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    
    // 分层优化策略：先尝试影响小的优化，再尝试影响大的优化
    if (metrics.fps < this.config.targetFPS * 0.5) {
      // 严重帧率问题：需要激进优化
      actions.push({ type: 'particleCount', value: Math.max(100, metrics.particleCount * 0.6), expectedGain: 25 });
      actions.push({ type: 'renderScale', value: Math.max(0.5, metrics.renderScale * 0.7), expectedGain: 20 });
      ...
      // 中等帧率问题：需要中度优化
      actions.push({ type: 'particleCount', value: Math.max(500, metrics.particleCount * 0.8), expectedGain: 15 });
      actions.push({ type: 'renderScale', value: Math.max(0.7, metrics.renderScale * 0.9), expectedGain: 10 });
      actions.push({ type: 'shadowQuality', value: Math.max(0, metrics.shadowQuality * 0.8), expectedGain: 10 });
    } else {
      // 轻微帧率问题：需要轻度优化
      actions.push({ type: 'particleCount', value: Math.max(1000, metrics.particleCount * 0.9), expectedGain: 5 });
      actions.push({ type: 'triangleCount', value: Math.max(100000, metrics.triangleCount * 0.9), expectedGain: 5 });
    }
   ...
   * 优化内存瓶颈
   */
  private optimizeMemoryBottleneck(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    ...
    // 降低纹理质量和内存使用
    actions.push({ type: 'textureMemory', value: Math.max(0.5, metrics.textureMemory * 0.7), expectedGain: 15 });
    
    // 减少粒子数量
    actions.push({ type: 'particleCount', value: Math.max(500, metrics.particleCount * 0.8), expectedGain: 10 });
   ...
   * 优化Draw Calls瓶颈
   */
  private optimizeDrawCallsBottleneck(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    ...
    // 减少对象数量
    actions.push({ type: 'objectCount', value: Math.max(100, metrics.objectCount * 0.8), expectedGain: 10 });
   ...
   * 优化三角形数量瓶颈
   */
  private optimizeTriangleCountBottleneck(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    ...
    // 减少高复杂度对象
    actions.push({ type: 'complexObjectCount', value: Math.max(10, metrics.complexObjectCount * 0.7), expectedGain: 15 });
   ...
   * 优化设备过热
   */
  private optimizeDeviceOverheating(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    ...
    // 减少粒子数量
    actions.push({ type: 'particleCount', value: Math.max(100, metrics.particleCount * 0.6), expectedGain: 15 });
    
    // 降低渲染分辨率
    actions.push({ type: 'renderScale', value: Math.max(0.5, metrics.renderScale * 0.7), expectedGain: 15 });
   ...
   * 优化低电量
   */
  private optimizeLowBattery(metrics: UnifiedPerformanceMetrics): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
   ...
   * 自适应优化：根据当前设备状态调整优化策略
   */
  private performAdaptiveOptimization(metrics: UnifiedPerformanceMetrics, analysis: any): Array<{ type: string; value: any; expectedGain: number }> {
    const actions: Array<{ type: string; value: any; expectedGain: number }> = [];
    ...
    switch (this.devicePerformanceLevel) {
      case DevicePerformanceLevel.LOW:
        ...
        actions.push({ type: 'particleCount', value: Math.max(500, metrics.particleCount * 0.5), expectedGain: 25 });
        actions.push({ type: 'shadowQuality', value: 0, expectedGain: 15 });
        break;
      case DevicePerformanceLevel.MEDIUM:
        // 中端设备：平衡优化
        actions.push({ type: 'renderScale', value: Math.min(0.9, metrics.renderScale * 0.95), expectedGain: 10 });
        actions.push({ type: 'particleCount', value: Math.max(1000, metrics.particleCount * 0.85), expectedGain: 15 });
        break;
      case DevicePerformanceLevel.HIGH:
        // 高端设备：质量优先
        actions.push({ type: 'renderScale', value: Math.min(1.0, metrics.renderScale * 1.05), expectedGain: -5 });
        actions.push({ type: 'particleCount', value: Math.min(20000, metrics.particleCount * 1.1), expectedGain: -10 });
        break;
      case DevicePerformanceLevel.ULTRA:
        // 超高端设备：极致质量
        actions.push({ type: 'renderScale', value: 1.0, expectedGain: -10 });
        actions.push({ type: 'particleCount', value: Math.min(50000, metrics.particleCount * 1.2), expectedGain: -15 });
        actions.push({ type: 'shadowQuality', value: 1.0, expectedGain: -5 });
        actions.push({ type: 'postProcessing', value: true, expectedGain: -15 });
        break;
    }
