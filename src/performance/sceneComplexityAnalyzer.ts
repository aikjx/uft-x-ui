...
export class SceneComplexityAnalyzer {
  ...
  private adjustBasedOnAnalysis(analysis: ComplexityAnalysis): void {
    ...
    if (needsAdjustment) {
      ...
      if (newMode !== this.performanceOptimizer.getCurrentMode()) {
        ...
        console.log(
          `场景复杂度调整: ${this.performanceOptimizer.getCurrentMode()} -> ${newMode}, 原因: ${analysis.contributingFactors.join(', ')}`
        )
      }
    }
  }
  ...
  private shouldAdjust(analysis: ComplexityAnalysis, performanceStatus: any): boolean {
    ...
    if (this.lastComplexityLevel !== analysis.level) {
      // 如果从低复杂度变为高复杂度，立即调整
      if (
        (this.lastComplexityLevel === 'low' && analysis.level === 'medium') ||
  ...
  // 确定最佳性能模式
  private determineOptimalPerformanceMode(
  ...
  private applyGradualAdjustment(targetMode: PerformanceMode): void {
    ...
    const newStrategy: OptimizationStrategy = {
      ...currentStrategy,
      particleCount: this.lerp(
        currentStrategy.particleCount || 0,
        targetStrategy.particleCount || 0,
        adaptationSpeed
      ),
      fieldResolution: this.lerp(
        currentStrategy.fieldResolution || 0,
        targetStrategy.fieldResolution || 0,
        adaptationSpeed
      ),
      renderScale: this.lerp(
        currentStrategy.renderScale || 1,
        targetStrategy.renderScale || 1,
        adaptationSpeed
      )
    }
