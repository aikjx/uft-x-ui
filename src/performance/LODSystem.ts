...
export class LODObject {
  ...
  private createSimplifiedInstance(quality: number): THREE.Mesh {
    ...
    const geometry = this.simplifyGeometry(this.baseGeometry, quality)
    const material = this.simplifyMaterial(this.baseMaterial, quality)
  ...
  private simplifyGeometry(
    ...
    // 根据质量调整几何体
    if (simplifiedGeometry instanceof THREE.BufferGeometry) {
      // 简化几何体：减少顶点数量
      const positionAttribute = simplifiedGeometry.attributes.position
      if (positionAttribute) {
        const originalCount = positionAttribute.count
        const targetCount = Math.max(3, Math.floor(originalCount * quality))
  ...
  private simplifyMaterial(
    ...
    const processSingleMaterial = (mat: THREE.Material): THREE.Material => {
      ...
      if (newMaterial instanceof THREE.MeshStandardMaterial) {
        // 根据质量调整材质特性
        newMaterial.metalnessMap = quality > 0.5 ? (newMaterial as any).metalnessMap : null
        newMaterial.roughnessMap = quality > 0.5 ? (newMaterial as any).roughnessMap : null
        newMaterial.normalMap = quality > 0.5 ? (newMaterial as any).normalMap : null
        newMaterial.displacementMap = quality > 0.7 ? (newMaterial as any).displacementMap : null

        // 降低反射率和粗糙度
        newMaterial.metalness = Math.max(0, newMaterial.metalness * quality)
        newMaterial.roughness = Math.min(1, newMaterial.roughness + (1 - quality) * 0.3)

        // 禁用不必要的功能
        newMaterial.aoMap = quality > 0.6 ? (newMaterial as any).aoMap : null
        newMaterial.emissiveMap = quality > 0.7 ? (newMaterial as any).emissiveMap : null
        newMaterial.specularMap = quality > 0.6 ? (newMaterial as any).specularMap : null

        // 简化着色器
        newMaterial.needsUpdate = true
      } else if (newMaterial instanceof THREE.MeshBasicMaterial) {
        // 简化基本材质
        newMaterial.map = quality > 0.5 ? newMaterial.map : null
        newMaterial.needsUpdate = true
      }
    ...
    if (Array.isArray(material)) {
      return material.map(processSingleMaterial)
    } else {
      return processSingleMaterial(material)
    }
  }
