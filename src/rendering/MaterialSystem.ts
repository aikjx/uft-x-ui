...
export interface MaterialConfig {
  type: 'basic' | 'phong' | 'lambert' | 'standard' | 'physical' | 'points' | 'line' | 'shadow' | 'toon' | 'meshDistance';
  color?: number | string;
  transparent?: boolean;
  opacity?: number;
...
export class MaterialSystem {
  ...
  private createMaterial(config: MaterialConfig): THREE.Material {
    ...
    if (config.useDynamicTexture) {
      ...
      if (config.clearcoatRoughnessMap) materialOptions.clearcoatRoughnessMap = config.clearcoatRoughnessMap;
    }
  ...
  private updateMaterialProperties(material: THREE.Material, config: MaterialConfig): void {
    ...
    if (material instanceof THREE.MeshStandardMaterial || material instanceof THREE.MeshPhysicalMaterial) {
      ...
      if (config.clearcoatRoughness !== undefined) (material as any).clearcoatRoughness = config.clearcoatRoughness;
      if (config.normalScale) (material as any).normalScale = config.normalScale;
      if (config.displacementScale !== undefined) (material as any).displacementScale = config.displacementScale;
      if (config.displacementBias !== undefined) (material as any).displacementBias = config.displacementBias;
      if (config.envMapIntensity !== undefined) (material as any).envMapIntensity = config.envMapIntensity;
    }
