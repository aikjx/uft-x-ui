...
export class TestPerformanceMonitor {
  ...
  endTest(testName: string, status: 'passed' | 'failed'): void {
    ...
    if (this.currentRun) {
      ...
      // 检测慢测试
      if (metric.duration > 1000) { // 超过1秒视为慢测试
        this.currentRun.slowTests.push(testName)
      }
      
      // 跟踪内存使用峰值
      this.currentRun.memoryUsage.max = Math.max(
        this.currentRun.memoryUsage.max,
        memoryEnd
      )
  ...
  private generateRecommendations(): string[] {
    ...
    if (this.currentRun.slowTests.length > 0) {
      recommendations.push(
        `检测到 ${this.currentRun.slowTests.length} 个慢测试，建议优化其性能`
      )
    }
    ...
    // 执行时间建议
    if (this.currentRun.duration > 30000) { // 超过30秒
