...
describe('Navbar - 导航菜单组件', () => {
  ...
  it('应该正确高亮公式可视化菜单项', async () => {
    // 导入react-router-dom模块
    const reactRouterDom = await import('react-router-dom');
    // 使用vi.spyOn来设置useLocation的返回值
    vi.spyOn(reactRouterDom, 'useLocation').mockReturnValue({ pathname: '/formulas', search: '', hash: '', state: null });
  ...
  it('应该正确高亮人工场技术菜单项', async () => {
    // 导入react-router-dom模块
    const reactRouterDom = await import('react-router-dom');
    // 使用vi.spyOn来设置useLocation的返回值
    vi.spyOn(reactRouterDom, 'useLocation').mockReturnValue({ pathname: '/artificial-field', search: '', hash: '', state: null });
  ...
  it('应该正确高亮交互探索菜单项', async () => {
    // 导入react-router-dom模块
    const reactRouterDom = await import('react-router-dom');
    // 使用vi.spyOn来设置useLocation的返回值
    vi.spyOn(reactRouterDom, 'useLocation').mockReturnValue({ pathname: '/interactive', search: '', hash: '', state: null });
