...
describe('PostProcessing - 后处理效果', () => {
  ...
  beforeEach(async () => {
    // 动态导入后处理模块
    EffectComposer = (await import('three/examples/jsm/postprocessing/EffectComposer'))
      .EffectComposer
    RenderPass = (await import('three/examples/jsm/postprocessing/RenderPass')).RenderPass
    UnrealBloomPass = (await import('three/examples/jsm/postprocessing/UnrealBloomPass'))
      .UnrealBloomPass
    ...
    GammaCorrectionShader = (await import('three/examples/jsm/shaders/GammaCorrectionShader'))
      .GammaCorrectionShader
  ...
  it('应该正确添加和配置 FilmPass', () => {
    ...
    const filmPass = new FilmPass(noiseIntensity, scanlineIntensity, scanlineCount, false)
  ...
  it('应该支持多种后处理效果的组合使用', () => {
    ...
    composer.addPass(
      new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.5, 0.1)
    )
