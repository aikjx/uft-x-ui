...
describe('ResourceManager 类', () => {
  ...
  it('should trigger garbage collection correctly', async () => {
    ...
    const resourceEntry = resourceManager['resources'].get(textureId)
    if (resourceEntry) {
      resourceEntry.metadata.lastUsedAt = Date.now() - 500 // 500毫秒前使用
      resourceEntry.metadata.isActive = false // 标记为不活跃
    }
    
    // 修改配置，设置一个非常短的TTL（100毫秒）
    resourceManager['config'].defaultTTL = 100
