...
// 自定义Three.js优化插件，减小Three.js打包体积
const threeOptimizerPlugin = {
  name: 'three-optimizer-plugin',
  enforce: 'pre' as const,
  resolveId(source) {
    // 优化Three.js的导入，只导入需要的模块
    if (source === 'three') {
      return {
        id: source,
        external: false
  ...
  transform(code, id) {
    // 替换Three.js的默认导出，只保留常用的模块
    if (id.includes('three/build/three.module.js')) {
      // 这里可以添加更复杂的Three.js优化逻辑
      return {
        code,
        map: null
      };
...
export default defineConfig({
  plugins: [
    ...
    threeOptimizerPlugin, // 添加Three.js优化插件
    // 添加gzip和brotli压缩
    viteCompression({
