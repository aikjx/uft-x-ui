...
export const useThreeScene = (options: UseThreeSceneOptions): ThreeSceneReturn => {
  ...
  const initializeFullScene = useCallback(() => {
    try {
      ...
      const renderer = new THREE.WebGLRenderer({
        ...
        preserveDrawingBuffer: false, // 禁用保留绘制缓冲区以提高性能
      ...
      renderer.info.autoReset = true; // 自动重置渲染器信息
      renderer.sortObjects = true; // 启用对象排序以提高渲染效率
      renderer.toneMapping = THREE.LinearToneMapping; // 使用高效的色调映射
  ...
  const animate = useCallback(() => {
    ...
    // 只在必要时更新控制器
    if (controlsRef.current && controlsRef.current.enabled && controlsRef.current.enableDamping) {
    ...
    try {
      // 只在场景需要更新时渲染
      if (sceneRef.current.userData.needsRender !== false) {
  ...
  const updateScene = useCallback((scene: THREE.Scene | null, deltaTime: number) => {
    ...
    try {
      if (!targetScene || !cameraRef.current) return;
      ...
      targetScene.traverse(object => {
        ...
        if (camera) {
          ...
          const isVisible = object.visible && 
            (!object.geometry || !object.geometry.boundingSphere || 
             frustum.intersectsSphere(object.geometry.boundingSphere.applyMatrix4(object.matrixWorld)));
          
          // 设置对象可见性
          object.frustumCulled = true;
          
          // 保存可见性状态，用于后续渲染优化
          object.userData.isVisible = isVisible;
        }
      });
      
      // 设置场景需要渲染的标志
      targetScene.userData.needsRender = sceneNeedsUpdate;
      ...
      // 确保场景仍然可以渲染
      if (targetScene) {
        targetScene.userData.needsRender = true;
      }
    }
  }, []);
