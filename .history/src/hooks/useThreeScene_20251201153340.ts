...
export const useThreeScene = (options: UseThreeSceneOptions): ThreeSceneReturn => {
  ...
  const clearScene = useCallback(() => {
    ...
    try {
      ...
      // 临时保存需要保留的对象（辅助线、灯光和内容对象）
      const objectsToKeep: THREE.Object3D[] = [];
      
      // 遍历所有子对象，保留重要的对象
      sceneRef.current.children.forEach(child => {
        // 保留辅助线
        if ((VISUALIZATION_CONFIG.showGrid && child.name === 'gridHelper') || 
      ...
      sceneRef.current.matrixAutoUpdate = false; // 保持禁用自动矩阵更新以提高性能
      
      // 恢复需要保留的对象
      objectsToKeep.forEach(obj => sceneRef.current?.add(obj));
