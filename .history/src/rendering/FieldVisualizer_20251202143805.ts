...
export class FieldVisualizer {
  ...
  constructor(scene: THREE.Scene, physicsEngine: PhysicsEngine, config: Partial<FieldVisualizerConfig> = {}) {
    this.scene = scene;
  ...
  private generateFieldLine(startPosition: THREE.Vector3): THREE.Line | null {
    ...
    for (let i = 0; i < maxPoints; i++) {
      ...
      const nextPosition = currentPosition.clone().add(
        fieldValue.clone().normalize().multiplyScalar(stepSize)
      );
   ...
   * 创建矢量箭头
   */
  private createVectorArrow(position: THREE.Vector3, fieldValue: THREE.Vector3, length: number): THREE.Line {
  ...
  private calculateFieldValue(position: THREE.Vector3): THREE.Vector3 {
    // 将Three.js Vector3转换为自定义Vector3
    const vector3 = new Vector3(position.x, position.y, position.z);
    const matrix4 = new Matrix4();
    
    let fieldValue: Vector3;
    
    switch (this.config.fieldType) {
      case 'gravitational':
        fieldValue = this.physicsEngine.calculateGravitationalField(vector3, matrix4);
        break;
      case 'electromagnetic':
        fieldValue = this.physicsEngine.calculateElectromagneticField(vector3, matrix4);
        break;
      case 'unified':
      default:
        fieldValue = this.physicsEngine.calculateUnifiedField(vector3, matrix4);
        break;
    }
    
    // 转换回Three.js Vector3
    return new THREE.Vector3(fieldValue.x, fieldValue.y, fieldValue.z);
  }
