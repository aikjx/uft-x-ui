...
export class ResourceManager {
  ...
  private predictedResources: Map<string, {
    priority: number;
    ...
    if (cachedResource && cachedResource.metadata.status === ResourceStatus.LOADING) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (updatedEntry.metadata.status === ResourceStatus.LOADED) {
          ...
          const error = updatedEntry.metadata.error || new Error(`Failed to load resource ${id}`);
      ...
      try {
        if (loader && loader.load) {
          ...
          loader.load(
            ...
            (error: any) => {
              this.handleResourceError(id, error instanceof Error ? error : new Error(String(error)));
              request.reject(error);
              resolve(); // 即使出错也继续处理队列
            }
          );
  ...
  private handleResourceError(id: string, error: Error): void {
    ...
    resourceEntry.metadata.loadDuration = resourceEntry.metadata.loadEndTime - (resourceEntry.metadata.loadStartTime || resourceEntry.metadata.loadEndTime);
  ...
  private updatePerformanceMetrics(metadata: ResourceMetadata): void {
    ...
    this.performanceMetrics.averageLoadTime = this.performanceMetrics.totalLoadTime / this.performanceMetrics.totalResourcesLoaded;
    ...
    while (this.totalCachedSize > this.config.maxCacheSize && this.resources.size > 0) {
      ...
      this.resources.forEach((resourceEntry, id) => {
        if (!resourceEntry.metadata.isPinned && !resourceEntry.metadata.isActive) {
          if (!leastUsedResource || resourceEntry.metadata.lastUsedAt < leastUsedResource.lastUsedAt) {
