...
export class ResourceManager {
   ...
   * 加载模型资源
   */
  private loadModel(loader: GLTFLoader, url: string, onProgress: (progress: number) => void): Promise<any> {
    return new Promise((resolve, reject) => {
      // 设置加载超时
      const timeoutId = setTimeout(() => {
        reject(new Error(`Model loading timed out after 30 seconds for URL: ${url}`));
      }, 30000);
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          resolve(gltf); // 返回完整的 GLTF 对象，而不仅仅是 scene
        },
        (progressEvent) => {
          const progress = progressEvent.lengthComputable ? progressEvent.loaded / progressEvent.total : 0;
          onProgress(progress);
            ...
            data.scene.traverse((object: any) => {
              ...
              if (object.material) {
                ...
                materials.forEach((material: any) => {
                  if (material.map) size += this.estimateResourceSize(material.map, 'texture');
                  if (material.normalMap) size += this.estimateResourceSize(material.normalMap, 'texture');
                  if (material.specularMap) size += this.estimateResourceSize(material.specularMap, 'texture');
                  if (material.roughnessMap) size += this.estimateResourceSize(material.roughnessMap, 'texture');
                  if (material.metalnessMap) size += this.estimateResourceSize(material.metalnessMap, 'texture');
                });
              }
            });
          } else if (data instanceof THREE.Scene) {
            // 直接处理 Scene 对象
            data.traverse((object: any) => {
              if (object.geometry) {
                size += object.geometry.attributes.position.count * 4 * 3; // 顶点数据
              }
              if (object.material) {
  ...
  private releaseIdleResources(): void {
    ...
    this.resources.forEach((resource, id) => {
      // 释放超过TTL且未固定的资源
      if (!resource.metadata.isPinned && (now - resource.metadata.lastUsedAt > ttl)) {
  ...
  public dispose(): void {
    ...
    console.log('ResourceManager disposed successfully');
  }
}
