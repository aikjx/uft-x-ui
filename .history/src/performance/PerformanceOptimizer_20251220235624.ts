...
export class PerformanceOptimizer {
  ...
  sortObjectsByPriority(objects: THREE.Object3D[]): THREE.Object3D[] {
    const isPerformanceMode = this.metrics.fps < this.config.targetFPS * 0.8
    return this.renderOptimizer.sortObjectsByPriority(objects, this.camera, isPerformanceMode)
  }
  ...
  private updateRendererSettings(): void {
    ...
    // 更新阴影贴图分辨率 - 在场景中遍历灯光进行设置
    this.scene.traverse(object => {
      if (object instanceof THREE.Light) {
        const light = object as THREE.DirectionalLight | THREE.SpotLight | THREE.PointLight
        if (light.shadow) {
          ...
          if (light.shadow.camera instanceof THREE.PerspectiveCamera) {
            light.shadow.camera.near = isPerformanceMode ? 0.5 : 0.1
            light.shadow.camera.far = isPerformanceMode ? 100 : 200
    ...
    this.renderer.antialias = !isPerformanceMode
    this.renderer.toneMappingExposure = isPerformanceMode ? 0.8 : 1.0

    // 优化场景渲染顺序
    this.renderOptimizer.optimizeRenderOrder(this.scene, this.camera)
  }

  /**
   * 优化场景几何体和材质
   */
  private optimizeSceneObjects(): void {
    const isPerformanceMode = this.metrics.fps < this.config.targetFPS * 0.8
    ...
    this.scene.traverse(object => {
      if (object instanceof THREE.Mesh) {
        ...
        if (object.geometry instanceof THREE.BufferGeometry) {
          ...
          if (positionAttribute && positionAttribute.count > 10000) {
            // 简化复杂几何体
            this.renderOptimizer.simplifyGeometry(object.geometry, isPerformanceMode ? 0.5 : 1.0)
          }
        }
        ...
        if (Array.isArray(object.material)) {
          object.material.forEach(material => {
            this.renderOptimizer.optimizeMaterial(material, isPerformanceMode)
          })
        } else {
          this.renderOptimizer.optimizeMaterial(object.material, isPerformanceMode)
        }
      ...
      if (object instanceof THREE.Points) {
        if (isPerformanceMode && object.geometry.attributes.position.count > 1000) {
          // 减少粒子数量
          this.particleOptimizer.reduceParticleCount(object, 0.5)
        }
      }
    })
  }
  ...
  getOptimizationSuggestions(): string[] {
    ...
    if (this.metrics.fps < this.config.targetFPS * 0.8) {
      suggestions.push(
        `当前FPS (${this.metrics.fps.toFixed(1)}) 低于目标FPS (${this.config.targetFPS})，建议降低场景复杂度或启用更激进的优化策略`
      )
    }
    ...
    if (this.metrics.memoryUsage > this.config.maxMemoryUsageMB * 0.8) {
      suggestions.push(
        `当前内存使用 (${this.metrics.memoryUsage.toFixed(0)}MB) 接近上限 (${this.config.maxMemoryUsageMB}MB)，建议释放未使用的资源`
      )
    }
    ...
    if (this.metrics.drawCalls > this.config.maxDrawCalls * 0.8) {
      suggestions.push(
        `当前绘制调用 (${this.metrics.drawCalls}) 接近上限 (${this.config.maxDrawCalls})，建议合并几何体或使用实例化渲染`
      )
    }
    ...
    if (this.config.enableParticleOptimization && this.metrics.activeObjects > 1000) {
      suggestions.push(
        `当前活跃对象数量 (${this.metrics.activeObjects}) 较多，建议启用更激进的粒子优化`
      )
    }
