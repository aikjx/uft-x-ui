...
export class PhysicsEngine implements IPhysicsEngine {
  ...
  calculateSpacetimeState(position: Vector3, time: number): SpacetimeState {
    ...
    // 检查缓存
    if (this.parameters.useOptimizedCalculations && this.curvatureCache.has(cacheKey)) {
      const curvature = this.curvatureCache.get(cacheKey)!;
    ...
    if (this.parameters.useOptimizedCalculations) {
      ...
      this.curvatureCache.set(cacheKey, curvature);
  ...
  calculateGravitationalField(position: Vector3, mass: number): Vector3 {
    ...
    // 优化计算：使用平方距离，避免开方
    // 兼容不同的Vector3实现，使用length()的平方而不是lengthSq()方法
  ...
  calculateUnifiedField(position: Vector3, time: number, mass: number, charge: number): UnifiedField {
    ...
    const electromagnetic = this.calculateElectromagneticField(position, charge, zeroVelocity);
