    ...
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        ...
        <div class="formula-info">
          ...
          <div class="info-section" v-if="formula.variables && formula.variables.length > 0">
            <h3 class="section-title">变量说明</h3>
            <div class="variables-list">
              <div v-for="variable in formula.variables" :key="variable.symbol" class="variable-item">
                <span class="variable-symbol">{{ variable.symbol }}</span>
                <span class="variable-description">{{ variable.description }}</span>
                <span class="variable-unit" v-if="variable.unit">({{ variable.unit }})</span>
              </div>
            </div>
          </div>
...
const viewRelatedFormula = (formulaId: number) => {
  emit('view-formula', formulaId)
}
...
const shareFormula = () => {
  const shareText = `张祥前统一场论 - ${props.formula.name}: ${props.formula.latex}`
