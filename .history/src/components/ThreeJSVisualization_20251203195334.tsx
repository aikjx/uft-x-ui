...
const ThreeJSVisualization: React.FC<ThreeJSVisualizationProps> = React.memo(({
  ...
  const initializePostProcessing = useCallback(() => {
    ...
    try {
      ...
      // 添加SMAA抗锯齿通道（如果设备性能允许）
      const devicePerformance = devicePerformanceAnalyzer.getPerformanceScore();
      if (devicePerformance > 60) {
      ...
      // 添加胶片颗粒效果，增强视觉质感
      if (devicePerformance > 50) {
        const filmPass = new FilmPass(0.3, 0.025, 256, false);
  ...
  }, [renderer, camera, getScene, bloomIntensity, bloomRadius, bloomThreshold]);
