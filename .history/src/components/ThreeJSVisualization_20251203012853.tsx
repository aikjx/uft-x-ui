...
const ThreeJSVisualization: React.FC<ThreeJSVisualizationProps> = React.memo(({
  ...
  const mergeGeometries = useCallback((scene: THREE.Scene) => {
    ...
    try {
      // 按材质分组几何体
      const materialsToGeometries = new Map<THREE.Material, THREE.Geometry[] | THREE.BufferGeometry[]>();
      ...
      scene.traverse((object) => {
        if (object instanceof THREE.Mesh && object.geometry && object.material) {
          ...
          materials.forEach((material) => {
            ...
            const geometries = materialsToGeometries.get(material)!;
      ...
      materialsToGeometries.forEach((geometries, material) => {
        ...
        if (geometries[0] instanceof THREE.BufferGeometry) {
          ...
          // @ts-ignore - BufferGeometryUtils可能没有正确导入，但Three.js内置支持
          mergedGeometry = THREE.BufferGeometryUtils.mergeBufferGeometries(geometries as THREE.BufferGeometry[]);
        } else {
          // 合并传统Geometry
          mergedGeometry = new THREE.BufferGeometry().fromGeometry(geometries[0] as THREE.Geometry);
          for (let i = 1; i < geometries.length; i++) {
            // @ts-ignore - BufferGeometryUtils可能没有正确导入，但Three.js内置支持
            const tempGeometry = new THREE.BufferGeometry().fromGeometry(geometries[i] as THREE.Geometry);
            // @ts-ignore - BufferGeometryUtils可能没有正确导入，但Three.js内置支持
            mergedGeometry = THREE.BufferGeometryUtils.mergeBufferGeometries([mergedGeometry, tempGeometry]);
          }
        }
      ...
      console.log('几何体合并完成，合并了', materialsToGeometries.size, '组几何体');
    } catch (error) {
    ...
    try {
      ...
      scene.traverse((object) => {
        if (object instanceof THREE.Mesh && object.geometry && !object.userData.hasLOD) {
          ...
          if (mediumGeometry instanceof THREE.BufferGeometry) {
            // 简化BufferGeometry
            // @ts-ignore - 可能需要使用外部库如three-simplify，但这里我们使用简单的缩放
            mediumGeometry.scale(0.5, 0.5, 0.5);
          }
          ...
          if (lowGeometry instanceof THREE.BufferGeometry) {
            // 进一步简化
            // @ts-ignore
            lowGeometry.scale(0.25, 0.25, 0.25);
          }
