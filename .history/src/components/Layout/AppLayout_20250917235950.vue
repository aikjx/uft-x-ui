      ...
      <div class="nav-container">
        <router-link to="/" class="nav-logo">
          <div
            v-motion
            :initial="{ scale: 1 }"
            :hovered="{ scale: 1.05, rotate: 5 }"
            class="logo-content"
            @mouseenter="$motion.apply('logo', 'hovered')"
            @mouseleave="$motion.apply('logo', 'initial')"
          >
        ...
        <div class="nav-links">
          ...
          >
            <div
              ...
              :initial="{ scale: 1 }"
              :hovered="{ scale: 1.05 }"
              @mouseenter="$motion.apply('navlink', 'hovered')"
              @mouseleave="$motion.apply('navlink', 'initial')"
            >
              {{ link.name }}
            </div>
          </router-link>
        ...
        <div class="nav-actions">
          <button
            ...
            :hovered="{ scale: 1.1 }"
            @click="toggleTheme"
            @mouseenter="$motion.apply('theme', 'hovered')"
            @mouseleave="$motion.apply('theme', 'initial')"
            class="theme-toggle"
          >
            {{ isDark ? '☀️' : '🌙' }}
          </button>
