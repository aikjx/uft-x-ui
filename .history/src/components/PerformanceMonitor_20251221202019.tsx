      ...
      >
        ...
        <div className="grid grid-cols-2 gap-y-3 gap-x-6 mb-3">
          ...
          >
            ...
            {/* 迷你趋势图 */}
            <div className="overflow-hidden mt-1 h-2 rounded-full bg-gray-700/50">
              <motion.div
                className={`h-full ${metrics.fps < 30 ? 'bg-red-500' : metrics.fps < 50 ? 'bg-yellow-500' : 'bg-green-500'}`}
          ...
          >
            ...
            <div className="flex gap-1 items-baseline">
              <span className="font-mono text-lg text-purple-400">{metrics.optimizationLevel || renderState.optimizationLevel}</span>
            </div>
            {/* 优化级别指示器 */}
            <div className="overflow-hidden mt-1 h-2 rounded-full bg-gray-700/50">
              <motion.div
                ...
                animate={{ width: `${((metrics.optimizationLevel || renderState.optimizationLevel) / 5) * 100}%` }}
                transition={{ duration: 0.3 }}
              />
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="flex gap-1 items-baseline">
              <span className="font-mono text-lg text-cyan-400">{metrics.renderTime.toFixed(1)}</span>
          ...
          >
            ...
            <span className="font-mono text-lg text-blue-400">{metrics.activeObjects || 0}</span>
          </motion.div>
          ...
          >
            ...
            <span className="font-mono text-lg text-pink-400">{metrics.shadowDrawCalls || 0}</span>
          </motion.div>
        </div>

        {/* 详细性能指标展开区 */}
        <motion.div
          className="pt-3 mt-3 text-xs text-gray-300 border-t border-blue-500/20"
        ...
        >
          <div className="grid grid-cols-2 gap-y-2 gap-x-4">
            <div className="flex justify-between">
              ...
              <span className={`font-mono ${metrics.cpuUsage > 80 ? 'text-red-400' : metrics.cpuUsage > 60 ? 'text-yellow-400' : 'text-green-400'}`}>
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-mono ${metrics.networkLatency > 200 ? 'text-red-400' : metrics.networkLatency > 100 ? 'text-yellow-400' : 'text-green-400'}`}>
                {metrics.networkLatency.toFixed(0)}ms
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">每帧时间:</span>
              <span className={`font-mono ${metrics.frameTime > 33 ? 'text-red-400' : metrics.frameTime > 16 ? 'text-yellow-400' : 'text-green-400'}`}>
