...
const ThreeJSVisualization: React.FC<ThreeJSVisualizationProps> = React.memo(({
  ...
  const animate = useCallback(() => {
    ...
    // 只在达到最小帧时间时才渲染
    if (frameTime >= minFrameTime || !lastFrameTimeRef.current) {
    ...
    if (renderer && currentScene && camera) {
      try {
        // 帧跳过逻辑
        const sceneComplexity = scene?.children.length || 0;
        const shouldSkip = performanceOptimizationManager.shouldSkipFrame(
          performanceStatsRef.current.frameCount,
          currentFPS || 60,
          {
            sceneComplexity,
            hasUserInteraction: false, // 可以根据实际交互情况设置
            isImportantFrame: false, // 可以根据实际情况设置
            frameTimeHistory: performanceStatsRef.current.fpsHistory
          }
        );
