...
import { smartRenderScheduler } from '../performance/smartRenderScheduler';
...
const ThreeJSVisualization: React.FC<ThreeJSVisualizationProps> = React.memo(({
  ...
  const [filmNoiseIntensity, setFilmNoiseIntensity] = useState(0.3);
  const [filmScanlineIntensity, setFilmScanlineIntensity] = useState(0.025);
  const [filmScanlineCount, setFilmScanlineCount] = useState(256);
  ...
  const initializeParticleSystem = useCallback(() => {
    ...
    // 根据设备性能调整粒子数量
    const performanceScore = devicePerformanceAnalyzer.getPerformanceScore();
    const maxParticles = performanceScore > 70 ? 100000 : performanceScore > 40 ? 50000 : 20000;
    const emissionRate = performanceScore > 70 ? 500 : performanceScore > 40 ? 250 : 100;
  ...
  }, [getScene, devicePerformanceAnalyzer]);
  ...
  const animate = useCallback(() => {
    ...
    // 计算场景复杂度
    const currentScene = getScene();
    const sceneComplexity = currentScene ? currentScene.children.length : 0;
    ...
    const frameInfo = {
      ...
      fps,
      isVisible,
      hasInteraction: false, // 可以根据实际交互情况设置
      sceneComplexity,
      priority: 0 // 默认优先级，将由智能渲染调度器计算
    };
    
    // 计算渲染优先级
    frameInfo.priority = smartRenderScheduler.calculateRenderPriority(frameInfo);
    
    // 决定是否跳过当前帧
    if (smartRenderScheduler.shouldSkipFrame(frameInfo)) {
      // 跳过帧
    ...
    // 渲染前准备
    smartRenderScheduler.preRender(frameInfo);
    
    // 发布渲染开始事件
    eventSystem.emit(APP_EVENTS.RENDER_START, {
      frameIndex: stats.frameCount,
      timestamp: currentTime,
      priority: frameInfo.priority
    });
    ...
    // 应用性能优化
    if (autoModeEnabled) {
      performanceOptimizationManager.update(clampedDeltaTime);
    ...
    if (renderer && currentScene && camera) {
      try {
        ...
        // 渲染后处理
        smartRenderScheduler.postRender(frameInfo, renderer);
    ...
    lastFrameTimeRef.current = currentTime;
    animationFrameRef.current = requestAnimationFrame(animate);
  }, [paused, onAnimationFrame, getScene, updateScene, controls, renderer, camera, currentMemory, autoModeEnabled, updateCullingState, adaptivePerformanceAdjustment, autoModeEnabled, devicePerformanceAnalyzer, resourceManager]);
