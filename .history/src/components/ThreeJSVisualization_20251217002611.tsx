...
const ThreeJSVisualization: React.FC<ThreeJSVisualizationProps> = React.memo(({
  ...
  useEffect(() => {
    setRenderState(prev => {
      // Only update if the paused state actually changed
      if (prev.isPaused !== paused) {
  ...
  // 使用React.memo优化子组件渲染，避免不必要的重渲染
  const MemoizedChildren = React.useMemo(() => {
    if (!children || !renderEngineRef.current) return null;
    
    const scene = renderEngineRef.current.getScene?.();
    const camera = renderEngineRef.current.getCamera?.();
    const renderer = renderEngineRef.current.getRenderer?.();
    const controls = renderEngineRef.current.getControls?.();
    
    if (scene && camera && renderer && controls) {
      return children({ scene, camera, renderer, controls });
    }
