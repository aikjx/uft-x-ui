<template>
  ...
  >
    ...
    <div 
      ...
      aria-label="数学公式"
    >
      <span v-if="inline" ref="formulaElement" v-html="sanitizedFormula"></span>
      <div v-else ref="formulaElement" v-html="sanitizedFormula"></div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async executeRender() {
      try {
        ...
        // 添加调试信息
        console.log('[MathFormula] 开始渲染公式:', this.formula)
        ...
        // 直接设置公式内容到DOM元素
        if (this.$refs.formulaElement) {
          const formulaContent = this.inline ? 
            `\\(${this.formula}\\)` : 
            `\\[${this.formula}\\]`;
          
          console.log('[MathFormula] 设置公式内容:', formulaContent)
