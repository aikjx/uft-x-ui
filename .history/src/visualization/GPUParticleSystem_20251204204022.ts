...
export class GPUParticleSystem {
  ...
  /**
   * 优化的粒子更新算法 - 提高大规模粒子场景的性能
   */
  private updateParticles(deltaTime: number): void {
    if (this.particleCount === 0) return;

    let hasUpdates = false;
    let hasLifeProgressUpdates = false;
    
    // 使用批量处理优化性能
    const batchSize = 1024; // 每批处理1024个粒子
    let activeParticles = this.particleCount;
    ...
    for (let i = 0; i < activeParticles; i++) {
      if (this.active[i] === 1) {
        ...
        if (this.ages[i] >= this.lifetimes[i]) {
          ...
          hasLifeProgressUpdates = true;
        }
      }
    }
    
    // 如果有粒子过期，使用更高效的压缩算法重新排列粒子
    if (hasUpdates) {
      // 使用双指针技术压缩粒子数组
      let writeIndex = 0;
      for (let i = 0; i < activeParticles; i++) {
        if (this.active[i] === 1) {
          // 如果当前粒子是活跃的，将其保留在writeIndex位置
          if (writeIndex !== i) {
            // 只在需要时才复制粒子数据
      ...
      this.buffersNeedUpdate[3] = true; // lifeProgress
      this.buffersNeedUpdate[4] = true; // active
    } else if (hasLifeProgressUpdates) {
      // 只有生命周期进度更新，只需要更新相关缓冲区
      this.buffersNeedUpdate[2] = true; // age
      this.buffersNeedUpdate[3] = true; // lifeProgress
    }
  ...
  private copyParticleData(fromIndex: number, toIndex: number): void {
    ...
    this.positions[toBase] = this.positions[fromBase];
    this.positions[toBase + 1] = this.positions[fromBase + 1];
    this.positions[toBase + 2] = this.positions[fromBase + 2];
    
    // 复制速度数据
    this.velocities[toBase] = this.velocities[fromBase];
    this.velocities[toBase + 1] = this.velocities[fromBase + 1];
    this.velocities[toBase + 2] = this.velocities[fromBase + 2];
    
    // 复制颜色数据
    this.colors[toBase] = this.colors[fromBase];
    this.colors[toBase + 1] = this.colors[fromBase + 1];
    this.colors[toBase + 2] = this.colors[fromBase + 2];
