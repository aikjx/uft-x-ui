...
export class GPUParticleSystem {
  ...
  public update(deltaTime: number) {
    ...
    // 清理所有非活跃粒子
    let writeIndex = 0;
    const initialCount = this.particleCount;
    
    for (let readIndex = 0; readIndex < initialCount; readIndex++) {
      if (this.active[readIndex] === 1) {
        // 粒子仍然活跃，保留它
        if (readIndex !== writeIndex) {
          // 交换粒子数据
          this.swapParticles(readIndex, writeIndex);
        }
        writeIndex++;
      } else {
        // 粒子已过期，添加到空闲池
        this.inactiveParticles.push(readIndex);
      }
    }
