...
export class GPUParticleSystem {
  ...
  private vertexShader = `
    ...
    void main() {
      ...
      // 点大小计算 - 简化计算，避免复杂的数学运算
      float finalSize = size * mix(startSize, endSize, lifeProgress);
      gl_PointSize = finalSize * (250.0 / -mvPosition.z);
      
      // 颜色计算 - 简化插值计算，提高性能
      vec3 interpolatedColor = mix(startColor, endColor, lifeProgress);
  ...
  private fragmentShader = `
    ...
    void main() {
      ...
      // 高效的软边缘计算 - 避免复杂函数调用
      float alpha = 1.0 - sqrt(rSquared);
      alpha = clamp(alpha, 0.0, 1.0); // 简化smoothstep
      
      // 预乘alpha优化 - 减少运算次数，提高透明度混合效率
      vec3 finalColor = vColor.rgb * alpha;
      float finalAlpha = vColor.a * alpha;
      
      // 根据生命周期添加额外的透明度衰减，使粒子淡出更自然
      finalAlpha *= (1.0 - vLifeProgress);
  ...
  public update(deltaTime: number) {
    ...
    if (this.performanceConfig.enableAutoEmissionRate) {
    ...
    if (particlesToEmit > 0) {
      // 限制每帧发射的粒子数量，避免性能突降
      const emitLimit = Math.min(particlesToEmit, 100); // 每帧最多发射100个粒子
    ...
    // 更新粒子年龄和状态 - 简化版，使用更直接的方式清理过期粒子
    let anyActiveChanged = false;
    let hasLifeProgressChanged = false;
    let activeCount = 0;
    ...
    // 第二步：如果有粒子过期，重新组织粒子数组
    if (activeCount < this.particleCount) {
      // 使用更直接的方式：创建一个活跃粒子索引数组
      const activeParticleIndices: number[] = [];
      
      // 收集所有活跃粒子的索引
      for (let i = 0; i < this.particleCount; i++) {
        if (this.active[i] === 1) {
          activeParticleIndices.push(i);
        } else {
          // 粒子已过期，添加到空闲池
      ...
      // 重新组织粒子数组，将活跃粒子移到前面
      for (let newIndex = 0; newIndex < activeParticleIndices.length; newIndex++) {
        const oldIndex = activeParticleIndices[newIndex];
        if (oldIndex !== newIndex) {
          this.swapParticles(oldIndex, newIndex);
        }
      }
      
      // 更新活跃粒子数量
      this.particleCount = activeParticleIndices.length;
      anyActiveChanged = true;
    }
    
    // 优化：只有当有粒子状态变化时才更新缓冲区
    if (hasLifeProgressChanged) {
      this.buffersNeedUpdate[2] = true; // age
      this.buffersNeedUpdate[3] = true; // lifeProgress
    }
    
    if (anyActiveChanged) {
      this.buffersNeedUpdate[0] = true; // position - 因为我们交换了粒子位置
      this.buffersNeedUpdate[1] = true; // velocity - 因为我们交换了粒子数据
      this.buffersNeedUpdate[4] = true; // active - 因为我们标记了粒子为非活跃
    }
