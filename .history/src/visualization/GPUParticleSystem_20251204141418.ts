...
export class GPUParticleSystem {
  ...
  public update(deltaTime: number) {
    ...
    // 更新粒子年龄和状态 - 直接在遍历中清理过期粒子
    let anyActiveChanged = false;
    let hasLifeProgressChanged = false;
    let particleIndex = this.particleCount - 1;
    
    // 使用反向遍历，避免交换粒子后跳过检查
    while (particleIndex >= 0) {
      if (this.active[particleIndex] === 1) {
        ...
        if (this.ages[particleIndex] >= this.lifetimes[particleIndex]) {
          ...
          if (particleIndex !== lastIndex) {
            // 交换当前粒子与最后一个活跃粒子的位置
            this.swapParticles(particleIndex, lastIndex);
            // 不递减索引，因为我们需要重新检查当前位置
          } else {
            // 添加到空闲粒子池
          ...
          // 计算生命周期进度
          this.lifeProgress[particleIndex] = this.ages[particleIndex] / this.lifetimes[particleIndex];
