...
export class GPUParticleSystem {
  ...
  public update(deltaTime: number) {
    ...
    if (this.particleCount > 1000) {
      // 批量处理粒子更新
      const batchSize = 100;
      for (let i = 0; i < this.particleCount; i += batchSize) {
        const endIndex = Math.min(i + batchSize, this.particleCount);
        this.updateParticleBatch(i, endIndex, deltaTime);
  ...
  private updateParticleBatch(startIndex: number, endIndex: number, deltaTime: number): void {
    ...
    // 批量清理过期粒子
    this.cleanupExpiredParticles(startIndex, endIndex);
  }
  
  /**
   * 批量清理过期粒子
   */
  private cleanupExpiredParticles(startIndex: number, endIndex: number): void {
    // 从后往前遍历，移除过期粒子
