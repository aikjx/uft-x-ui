...
export class GPUParticleSystem {
  ...
  private buffersNeedUpdate: boolean[] = [false, false, false, false]; // position, velocity, age, active
  ...
  private vertexShader = `
    ...
    void main() {
      ...
      gl_PointSize = size * sizeInterp * scale;
  ...
  private fragmentShader = `
    ...
    void main() {
      ...
      // 软边缘 - 优化：使用线性插值替代smoothstep
      float alpha = 1.0 - 2.0 * sqrt(distSq);
  ...
  private emitParticleAt(index: number) {
    ...
    this.buffersNeedUpdate[3] = true; // active
  }
