...
export class GPUParticleSystem {
  ...
  public update(deltaTime: number) {
    ...
    // 更新粒子年龄和状态 - 简化版，确保所有粒子都能被正确处理
    let anyActiveChanged = false;
    let hasLifeProgressChanged = false;
    
    // 直接计算哪些粒子应该被保留
    const remainingParticles: number[] = [];
    ...
    // 如果有粒子过期，重新组织粒子数组
    if (remainingParticles.length < this.particleCount) {
      // 遍历所有剩余粒子，将它们移到前面
      for (let newIndex = 0; newIndex < remainingParticles.length; newIndex++) {
        const oldIndex = remainingParticles[newIndex];
      ...
      // 更新活跃粒子数量
      this.particleCount = remainingParticles.length;
      anyActiveChanged = true;
    }
