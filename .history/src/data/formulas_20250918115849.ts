...
export const simpleFormulas: SimpleFormula[] = [
  {
    id: 1,
    name: "时空同一化方程",
...
// 转换函数：将SimpleFormula转换为Formula
export const convertToFormula = (simple: SimpleFormula): Formula => ({
  id: simple.id.toString(),
  name: simple.name,
  description: simple.description,
  equation: simple.latex,
  categoryId: simple.category,
  difficulty: 'intermediate' as const,
  keywords: [simple.category, simple.name],
  parameters: [],
  visualization: {
    type: '2d' as const,
  ...
  theory: {
    ...
    significance: simple.description,
    applications: [simple.category]
  },
  examples: [],
  references: []
});
...
// 导出转换后的公式数组
export const formulas: Formula[] = simpleFormulas.map(convertToFormula);