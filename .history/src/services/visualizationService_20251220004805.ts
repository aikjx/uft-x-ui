...
export class VisualizationService {
  ...
  private createSpacetimeDistortion(scene: THREE.Scene, parameters: any): void {
    ...
    plane.userData.visualizationType = 'spacetime'
    scene.add(plane)
  ...
  private createSpiralParticles(scene: THREE.Scene, parameters: any): void {
    ...
    // 标记为动态对象
    particleSystem.userData.isDynamic = true
    scene.add(particleSystem)
  }

  private createGravityFieldLines(
    scene: THREE.Scene,
  ...
  private createFieldInteractions(scene: THREE.Scene, parameters: any): void {
    ...
    // 标记为动态对象
    particles.userData.isDynamic = true
    scene.add(particles)
  }
  ...
  public updateVisualization(scene: THREE.Scene, deltaTime: number, parameters: any): void {
    // 限制deltaTime，避免帧率过高时计算量过大
    const limitedDeltaTime = Math.min(deltaTime, 0.1)

    // 只更新标记为动态的对象，避免遍历整个场景
    scene.traverse(object => {
      // 检查对象是否有userData.isDynamic标记
      if (object.userData.isDynamic) {
        if (object instanceof THREE.Points) {
          // 粒子动画
          this.animateParticles(object, limitedDeltaTime)
        } else if (object instanceof THREE.Line) {
          // 场线动画
          this.animateFieldLines(object, limitedDeltaTime)
        } else if (object instanceof THREE.Mesh) {
          // 网格动画（如时空扭曲）
          if (object.userData.visualizationType === 'spacetime') {
            this.applySpacetimeDistortion(object, parameters.curvature || 0.5, Date.now() * 0.001)
          }
        }
      }
    })
  ...
  private animateParticles(particles: THREE.Points, deltaTime: number): void {
    ...
    // 优化：每帧只更新一部分粒子，减少计算量
    const updateStep = Math.max(1, Math.floor(positions.length / (3 * 100))) // 每次更新最多100个粒子
  ...
  private animateFieldLines(line: THREE.Line, deltaTime: number): void {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      const wave = Math.sin(positions[i] + time) * 0.1
      positions[i + 1] = wave
    }
