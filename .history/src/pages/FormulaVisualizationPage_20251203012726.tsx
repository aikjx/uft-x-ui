...
const FormulaVisualizationPage: React.FC = () => {
  ...
  return (
    <motion.div
      className="relative w-full h-screen flex flex-col bg-[#0a0a14] p-0 m-0 overflow-hidden"
    ...
    >
        <div className="flex flex-col gap-0 px-0 mx-0 lg:flex-row lg:gap-0 w-full h-full">
          ...
          <motion.div
            className={`${isMobileMenuOpen ? 'fixed inset-0 z-50' : 'lg:w-60'} bg-gradient-to-br from-[#121228] to-[#0f0f20] rounded-r-2xl p-2 border border-blue-900/30 h-fit sticky top-0 lg:max-h-[100vh] overflow-hidden flex flex-col shadow-xl shadow-blue-900/5 transition-all duration-300 z-10 lg:flex"
            variants={formulaVariants}
            animate={isMobileMenuOpen ? { x: 0, opacity: 1 } : { x: -10, opacity: 0 }}
            style={{ display: isMobileMenuOpen ? 'flex' : 'none', lg: { display: 'flex' } }}
          >
            {/* 移动端菜单按钮 */}
