...
// 模拟react-router-dom模块
vi.mock('react-router-dom', () => {
  // 创建基本的location对象
  const mockLocation = { pathname: '/', search: '', hash: '', state: null };
  
  return {
    // 导出原始的BrowserRouter和MemoryRouter以确保路由上下文正常工作
    BrowserRouter: vi.importActual('react-router-dom').BrowserRouter,
    MemoryRouter: vi.importActual('react-router-dom').MemoryRouter,
    Routes: vi.importActual('react-router-dom').Routes,
    Route: vi.importActual('react-router-dom').Route,
