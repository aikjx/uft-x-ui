...
import { ParticleEmitter, ParticleSystemManager, ObjectPool, Particle } from '@/rendering/ParticleSystem';
...
describe('Particle', () => {
  ...
  it('should update correctly', () => {
    ...
    expect(particle.position.y).toBe(0.5); // 位置 = 初始位置 + 0.5 * 加速度 * 时间^2
    expect(particle.position.z).toBe(0);
...
describe('ParticleEmitter', () => {
  ...
  it('should update particles correctly', () => {
    // 发射一些粒子
    particleEmitter.update(1);
    const initialCount = particleEmitter.getActiveCount();
    
    // 再次更新，粒子应该被更新
    particleEmitter.update(0.1);
    
    // 粒子数量应该减少或者保持不变（取决于粒子生命周期）
    expect(particleEmitter.getActiveCount()).toBeLessThanOrEqual(initialCount);
  });
