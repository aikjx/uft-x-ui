...
describe('Navbar - 导航菜单组件', () => {
  ...
  it('应该正确渲染所有导航菜单项', () => {
    ...
    expect(desktopNavItems).toHaveLength(5)
  ...
  it('应该正确高亮当前活动菜单项', () => {
    ...
    const { rerender } = renderWithRouter(
      <Router initialEntries={['/formulas']}>
        <Navbar />
      </Router>
    )
    ...
    // 切换到另一个路径
    rerender(
      <Router initialEntries={['/artificial-field']}>
        <Navbar />
      </Router>
    )
  ...
  it('应该在不同设备尺寸下有正确的布局', () => {
    ...
    // 导航项应该隐藏
    const formulasLink = screen.getByText('公式可视化')
    expect(formulasLink).not.toBeVisible()
  })
  ...
  it('应该有正确的视觉反馈', () => {
    ...
    // 检查悬停效果
    fireEvent.mouseEnter(formulasLink!)
    expect(formulasLink).toHaveClass('hover:text-blue-300')
    
    // 检查点击效果
    fireEvent.mouseDown(formulasLink!)
    // 按钮应该有按下效果
  })
})
