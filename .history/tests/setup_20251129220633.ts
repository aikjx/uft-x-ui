...
// 模拟react-router-dom模块，确保useLocation返回正确的location对象
vi.mock('react-router-dom', () => {
  ...
  const MockNavLink = ({ children, to, ...props }: any) => {
    return React.createElement('a', { href: to, ...props }, children);
  };
  
  const MockMemoryRouter = ({ children, initialEntries }: any) => {
    // 设置初始路径
    if (initialEntries && initialEntries.length > 0) {
      mockLocation = { ...mockLocation, pathname: initialEntries[0] };
    }
  ...
  const MockHashRouter = ({ children }: any) => {
    return React.createElement('div', null, children);
  };
  
  const MockRoutes = ({ children }: any) => {
    return React.createElement('div', null, children);
  };
  
  const MockRoute = ({ children }: any) => {
    return React.createElement('div', null, children);
  };
  
  const MockOutlet = () => {
    return React.createElement('div', null, 'Outlet');
  };
  
  // 确保useLocation返回正确的location对象
  const useLocationMock = vi.fn(() => mockLocation);
  ...
  return {
    ...
    useResolvedPath: vi.fn(() => ({ pathname: '/', search: '', hash: '' })),
    useHref: vi.fn(() => '/'),
...
vi.mock('framer-motion', () => {
  // 创建一个简单的motion组件模拟
  const mockMotionComponent = (Component: any) => {
    return Component;
  };
  
  // 添加create方法
  mockMotionComponent.create = (Component: any) => Component;
  
  // 模拟常用的motion组件
  mockMotionComponent.div = (props: any) => React.createElement('div', props);
  mockMotionComponent.a = (props: any) => React.createElement('a', props);
  mockMotionComponent.button = (props: any) => React.createElement('button', props);
  mockMotionComponent.span = (props: any) => React.createElement('span', props);
  
  return {
    motion: mockMotionComponent,
    ...
    useInView: vi.fn(() => [true, vi.fn()]),
    useScroll: vi.fn(() => ({ scrollY: 0, scrollX: 0 })),
    useTransform: vi.fn(() => 0),
    useMotionValue: vi.fn(() => ({ get: () => 0, set: vi.fn() })),
    useMotionValueEvent: vi.fn(() => vi.fn()),
