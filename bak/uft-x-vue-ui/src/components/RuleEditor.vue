      ...
      <t-form-section title="规则配置">
        ...
        <t-form-item label="问题类别" name="category" required>
          <t-select v-model="rule.category">
            <t-option v-for="category in categories" :key="category" :value="category" :label="getCategoryText(category)" />
          </t-select>
        </t-form-item>
        
        <t-form-item label="严重程度" name="severity" required>
          <t-select v-model="rule.severity">
            <t-option v-for="severity in severities" :key="severity" :value="severity" :label="getSeverityText(severity)" />
          </t-select>
        </t-form-item>
      ...
      <t-form-section title="匹配条件">
        <div class="conditions-section">
          <div class="conditions-header">
            ...
            <t-button size="small" @click="addCondition">
          ...
          <div v-else class="conditions-list">
            <div v-for="(condition, index) in rule.conditions" :key="index" class="condition-item">
              <div class="condition-header">
                ...
                <t-button size="small" variant="text" theme="danger" @click="removeCondition(index)">
                  <t-icon name="delete" />
                </t-button>
              </div>
              ...
              <div class="condition-content">
                <t-form-item label="条件类型">
                  <t-select v-model="condition.type" @change="updateConditionTemplate(condition)">
                    <t-option v-for="type in conditionTypes" :key="type" :value="type" :label="type" />
                  </t-select>
                </t-form-item>
                ...
                <t-form-item label="参数">
                  <div class="parameters-section">
                    <div v-for="(value, key) in condition.parameters" :key="key" class="parameter-item">
                      <span class="parameter-key">{{ key }}:</span>
                      <t-input v-model="condition.parameters[key]" size="small" />
                      <t-button size="small" variant="text" @click="removeParameter(condition, key)">
                        <t-icon name="close" />
                      </t-button>
                    </div>
...
import { ProgrammingLanguage, IssueCategory, IssueSeverity, RuleCondition } from '../types/code-optimization'
...
const conditionTypes = ['ast_pattern', 'complexity_threshold', 'length_threshold', 'depth_threshold']
...
const getCategoryText = (category: IssueCategory) => {
  switch (category) {
    case IssueCategory.PERFORMANCE: return '性能'
    case IssueCategory.MEMORY: return '内存'
    case IssueCategory.BEST_PRACTICE: return '最佳实践'
    case IssueCategory.CODE_STYLE: return '代码风格'
    case IssueCategory.BUG: return '错误'
    case IssueCategory.SECURITY: return '安全'
    default: return '其他'
  }
}
