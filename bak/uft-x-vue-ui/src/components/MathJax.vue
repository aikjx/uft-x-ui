<template>
  <div ref="mathElement" class="mathjax-container" :class="{ 'mathjax-error': hasError }">
    <!-- 错误状态显示 -->
...
<script setup lang="ts">
  ...
  function configureMathJax() {
    const config: any = {
      ...
      // 扩展配置
      extensions: ['tex2jax.js', 'mml3.js', 'content-mathml.js'],
      // 处理配置
      tex2jax: {
        inlineMath: [
          ['$', '$'],
  ...
  function handleMathJaxError(error: Error) {
    ...
    errorMessage.value = props.throwError ? `公式错误: ${errorMsg}` : '公式渲染失败，请检查公式语法'
  ...
  function optimizeRenderedFormula(element: HTMLElement) {
    ...
    svgElements.forEach(svg => {
      ...
      svg.setAttribute(
        'aria-label',
        `数学公式: ${props.formula.substring(0, 50)}${props.formula.length > 50 ? '...' : ''}`
      )
    })
  }
