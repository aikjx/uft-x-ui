  ...
  <div class="performance-monitor" :class="[monitorPosition, { minimized: isMinimized }]">
    ...
    <div class="monitor-header" @click="toggleMinimize">
      ...
      <div class="header-right">
        <button class="control-btn" @click.stop="toggleMonitoring" :title="isMonitoring ? '停止监控' : '开始监控'">
          {{ isMonitoring ? '⏹️' : '▶️' }}
        </button>
        <button class="control-btn" @click.stop="toggleMinimize" :title="isMinimized ? '展开' : '最小化'">
          {{ isMinimized ? '⬆️' : '⬇️' }}
        </button>
        <button class="control-btn" @click.stop="closeMonitor" title="关闭">
          ❌
        </button>
      </div>
    </div>
...
.performance-score.excellent { background: rgba(76, 175, 80, 0.3); }
.performance-score.good { background: rgba(255, 193, 7, 0.3); }
.performance-score.fair { background: rgba(255, 152, 0, 0.3); }
.performance-score.poor { background: rgba(244, 67, 54, 0.3); }
...
.status-indicator.good { background: #FFC107; }
...
.minimized-score.excellent { background: rgba(76, 175, 80, 0.3); }
.minimized-score.good { background: rgba(255, 193, 7, 0.3); }
.minimized-score.fair { background: rgba(255, 152, 0, 0.3); }
.minimized-score.poor { background: rgba(244, 67, 54, 0.3); }

.minimized-fps, .minimized-memory {
  font-size: 0.8em;
  opacity: 0.8;
}
