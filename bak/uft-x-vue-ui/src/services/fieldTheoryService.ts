...
export class FieldTheoryService {
  ...
  private calculateFieldForce(
    ...
    // 简单的缓存机制，基于时间和参数组合
    if (cached && this.currentTime - cached.timeStamp < 0.016) {
      // 缓存60fps的一帧
    ...
    switch (fieldType) {
      ...
      case FieldType.WAVE: {
        ...
        const phase =
          TWO_PI * params.waveFrequency * this.currentTime - distance / params.waveLength
   ...
   * 计算势垒电势
   */
  calculateBarrierPotential(position: Vector3, params: FieldParams): number {
   ...
   * 计算量子粒子在势垒附近的波函数
   */
  calculateQuantumWaveFunction(position: Vector3, time: number, params: FieldParams): Vector3 {
    const { TWO_PI, SQRT_2 } = FieldTheoryService.MATH_CONSTANTS
    const barrierCenter = new Vector3(
      params.barrierPositionX,
      params.barrierPositionY,
    ...
    const envelope = Math.exp(
      (-distance * distance) / (2 * params.wavePacketWidth * params.wavePacketWidth)
    )
   ...
   * 量子隧穿力计算
   */
  private calculateTunnelingForce(particle: Particle, params: FieldParams): Vector3 {
    const { TWO_PI, SQRT_2 } = FieldTheoryService.MATH_CONSTANTS
    const force = new Vector3(0, 0, 0)
